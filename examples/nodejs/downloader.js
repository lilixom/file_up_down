    ...
    if (fs.existsSync(filename)) {
        if (position <= fs.statSync(filename).size) {
            if (blockSize == 0) {
                ...
                //比较重要地方对于下载文件统一修改cookie

                var cookie = "downloadToken=" + formdata["downloadToken"]

                res.writeHead(200, {
                    "Content-Type": "application/zip;text/plain",
                    "Content-Disposition": "attachment; filename=test.zip",
                    "Content-Length": blockSize,
                    'Set-Cookie': cookie, // 向客户端设置一个Cookie
                    "p3p": "CP=\"IDC DSP COR ADM DEVi TAIi PSA PSD IVAi IVDi CONi HIS OUR IND CNT\"" //ie 无法修改iframe 解决 翻案
                });
